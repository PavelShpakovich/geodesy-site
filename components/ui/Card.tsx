...
function CardHeader({ className, ...props }: React.ComponentProps<'header'>) {
  return (
    <header
      ...
      className={cn(
        '@container/card-header flex flex-col gap-2 has-data-[slot=card-action]:flex-row has-data-[slot=card-action]:items-start has-data-[slot=card-action]:justify-between',
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<'footer'>) {
  return <footer data-slot='card-footer' className={cn('flex items-center', className)} {...props} />;
}
